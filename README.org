...
borrowing another row's.  A terminal draws with characters, and a
character needs a row, so it cannot keep a row the window shows above
the text outside the box, and a mode line always closes the box
there; a graphic display draws those edges on the rows themselves and
follows the options exactly.
...
terminal.  On a graphic display the corners bend on the rows the box
draws itself, the edge rows it writes into a row your window leaves
free; the ends on a row your window brought stay square, because an
image cannot be pinned to such a row's edge —
[[file:docs/implementation.org][docs/implementation.org]] has the measurements:
