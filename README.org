...
buffer.  The sides are drawn in the margins of the window, on a
graphic display as in a terminal.  The horizontal edges are drawn on
...
back when it goes.  It does not change what your header line and your
mode line show, and =window-box-padding= buys air between the box and
the text.

[[file:img/demo.gif]]

The picture below shows two side windows with a box.  The frame comes
from =make gui=, which also measures the box.
...
| =window-box-side-characters=  | the two characters of the sides on a graphic display  |
...
pixel because an overline cannot be thicker, and a side is a hairline
at the edge of its column of margin.

That hairline is a character of your font.  A font that draws its
block characters over the whole line gives an unbroken line; one that
draws them shorter leaves the pixels between the lines bare, and the
side reads as dashed.  Set the option to nil for a column of the
margin in the color of the box, which is unbroken in any font:
...
- A hairline side is a character of your font, so a font that draws
  its block characters shorter than a line leaves bare pixels between
  the lines.  Measured at the same size: FiraCode draws it over the
  whole line, Noto Sans Mono four pixels short.  Set
  =window-box-side-characters= to nil for a column of the margin,
  which is unbroken in any font.
- The box takes one column of the window on each side, and
  =window-box-padding= more.  A buffer that uses its margins for its
  own text keeps them and gets the horizontal edges alone.
