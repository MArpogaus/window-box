...
- A line carries one =line-prefix=, so a side of the box and a gutter
  ...
  where the box draws the two as one string.  It does that for every
  region of the buffer's own, an overlay for each, unless
  =window-box-compose-prefix= caps them; past a cap the gutter is
  drawn and the sides are not.  New gutter is
