...
- On a graphic display the sides live in the fringes, so a line
  whose fringe shows an indicator of its own — magit's section
  arrows, a continuation arrow — shows that instead of the side
  there, and a frame whose fringes are switched off shows no sides at
  all.  A terminal draws the sides in one column of margin per side;
  a buffer that uses its margins for its own text keeps them, and
  such a window gets the horizontal edges alone there.
