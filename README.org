...
| =window-box-side-characters=  | a pair of characters for the sides, or nil for a column |
...
The horizontal edges are one pixel.  An overline cannot be thicker.  A
side is one column of the margin, which the box fills with the color
of the box.
...
- A side is one column of the margin.  Two characters in
  =window-box-side-characters= draw a hairline instead, and how much
  of its cell a character covers is then the font's business: a font
  whose block characters are shorter than a line draws the sides with
  a gap at each line.
