...
buffer.  On a graphic display the box is one pixel wide.  In a
terminal the box is made of box drawing characters.

The mode does not change what your header line and your mode line
show.  It draws its edges around them, or between them and the text.
...
The box is always one pixel wide.  An overline cannot be thicker, and
the other edges keep to that width.
...
- In a terminal the side edges are characters on the lines of text.
  They stop where the buffer ends.  On a graphic display the side
  edges are fringes, and fringes run the full height of the window.
...
- On a graphic display Emacs draws its own border in the first pixel
  ...
  covers the fringe of the box.  A line is there in each case, but it
  has the color of the frame and not the color of the box.
- The color of the fringes belongs to the buffer, because a face
  remap does.  Fringe indicators such as continuation arrows get the
  color of the box.
...
- A scroll bar is outside the fringe, and the right edge of the box
  is then inside the scroll bar.  Turn the scroll bar off for a clean
  box.
- The box makes the fringes of the window one pixel wide.  With the
  default fringes of eight pixels the text moves by seven pixels.
  =(fringe-mode 1)= keeps the text where it is.
...
- buffer-box draws the side edges with overlays, which stop at the
  ...
  in a terminal, and the fringes on a graphic display.  Both reach
  the rows below the last line of text.
