...
The hairline is a character, and how tall its ink is, is the font's
business.  Where a font draws it shorter than a line, the pixels
between the lines would stay bare, so the box asks the font and fills
the whole column with its color instead.  Nothing to configure: the
box is a hairline where the font can carry one and an unbroken column
where it cannot.
...
- A hairline side is a character, so a font that draws its block
  characters one pixel short of the line leaves one bare pixel at each
  line.  Where a font is shorter than that, the box draws the whole
  column instead of a dashed line.  Set
  =window-box-side-characters= to nil for the column always.
