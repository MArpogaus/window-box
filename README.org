...
| =window-box-yield-prefix=     | give the sides up to a buffer that draws its own     |
...
- A line carries one =line-prefix=, so a side of the box and a gutter
  the buffer draws cannot both be on it: dirvish's subtree guide,
  =org-indent-mode=, a shell that indents its output.  Such a buffer
  keeps its own gutter and the box draws its horizontal edges alone;
  the sides come back when the gutter goes.  Set
  =window-box-yield-prefix= to nil for the other trade, where the
  sides win and the gutter waits under the box.
