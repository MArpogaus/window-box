...
where you can read them, and it leaves the margins to their owners: a
buffer that keeps text in its margins — magit's log holds its dates
in one — keeps them, inside the box.  The mode does not change what
...
- On a graphic display the sides live in the fringes, and a fringe
  ...
  of its own shows that instead of the side, and a frame whose
  fringes are switched off shows no sides at all.  Every common
  indicator has a home outside the fringes — that is where the box's
  margins-stay-yours rule pays off:
...
- A terminal draws the sides in one column of margin per side; a
  buffer that uses its margins for its own text keeps them, and such
  a window gets the horizontal edges alone there.
