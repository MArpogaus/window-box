...
The horizontal edges are one pixel.  An overline cannot be thicker.
A side is one column of margin, and how much of that column it fills
is the character it is drawn with: the default pair is a line at the
edge of its own cell.
...
- A side is a character in a margin, and how much of its cell a
  character covers is the font's business.  A font whose block
  characters are shorter than a line draws the sides with a gap at
  each line.  Set =window-box-side-characters= to nil for a solid
  column, which every font gives you.
